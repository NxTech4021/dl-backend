...
export const divisionNotifications = {
  ...
  created: (divisionName: string, seasonName: string, adminName?: string): NotificationPayload => ({
    type: NOTIFICATION_TYPES.DIVISION_CREATED,
    category: getCategoryForNotificationType(NOTIFICATION_TYPES.DIVISION_CREATED),
    title: 'Division Created',
    message: `Division ${divisionName} has been created for ${seasonName}${adminName ? ` by ${adminName}` : ''}`,
  ...
  removed: (divisionName: string, seasonName: string, reason?: string): NotificationPayload => ({
    ...
    message: `You've been removed from ${divisionName} in ${seasonName}${reason ? `: ${reason}` : ''}`,
    metadata: { divisionName, seasonName, reason },
  }),

  transferred: (fromDivision: string, toDivision: string, seasonName: string): NotificationPayload => ({
